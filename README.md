...
No cooperativismo, cada associado possui um voto e as decisões são tomadas em assembleias, por votação. Imagine que você deve criar uma solução para um site web para gerenciar e participar dessas sessões de votação.
...
- As pautas e os votos são persistidos e que não são perdidos com o restart da aplicação.

### Como esta versionada a API da aplicação? Que estratégia usada?
